<script lang="ts">
    ...
    export let initialSets: TcgApiSet[];
    ...
    import { useFindManyTcgApiSet } from "$lib/hooks";
    ...
    export let tcgApiSetsQuery: ReturnType<typeof useFindManyTcgApiSet>;

    $: tcgApiSetsQuery = useFindManyTcgApiSet({
        where:
            requiresCardUpdate === "all"
                ? {}
                : { requiresCardUpdate: requiresCardUpdate === "true" },
        orderBy: { releaseDate: "asc" },
    });

    const columns: { [id: string]: ColumnDef<TcgApiSet, any>[] } = {
        mobile: [
            {
        ...
        desktop: [
            ...
            {
                accessorKey: "requiresCardUpdate",
                cell: (info: any) =>
                    info.row.original.requiresCardUpdate ? "Stale" : "Ok",
                header: () => "RequiresCardUpdate",
            },
        ],
    };
    ...
    let options = writable<TableOptions<TcgApiSet>>({
        data: initialSets,
